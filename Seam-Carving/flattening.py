...
def neighbors(img, row, col):
    """
    Get the neighboring pixels for a given pixel in an image.
    ...
    tuple of numpy arrays
        The RGB values of the neighboring pixels.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Seam carving image reduction tool")
    parser.add_argument('input_file', help="Path to the input image")
    parser.add_argument('-a', '--axis', required=True,
                        choices=['x', 'y'],
                        help="Axis to shrink the image on (x: width, y: height).")
    parser.add_argument('-p', '--pixels', type=int, required=True,
                        help="Number of pixels to remove.")
    parser.add_argument('-o', '--output', help="Output file name.")
    parser.add_argument('-i', '--interval', type=int,
                        help="Save intermediate images at given intervals.")
    parser.add_argument('-b', '--border', action='store_true',
                        help="Add a black border to maintain the original size.")
    parser.add_argument('-s', '--show_seam', action='store_true',
                        help="Visualize the removed seam in intermediate images.")
    ...
    if args.border:
        cropped_img = pad_img(cropped_img, *img.shape[:2])

    img_to_save = Image.fromarray(cropped_img)
    if img_to_save.mode == 'RGBA':
        img_to_save = img_to_save.convert('RGB')
    img_to_save.save(args.output)

    print(f"\nImage {args.input_file} cropped by {args.pixels} pixels along the {args.axis}-axis and saved as {args.output}\n")
